...
"""
Optional custom-network utilities ported from the pypsa-zambia fork of
PyPSA-Earth: custom transmission line types and cross-border interconnector
modelling. Both are opt-in via config (``lines.custom_line_types`` and
``validation.interconnectors.enable``) and are no-ops for any run that
doesn't set them.
"""
...
def find_nearest_bus(n, lat, lon, home_countries, distance_crs="EPSG:20935"):
    """Return the nearest modelled-country bus to a given latitude and longitude.

    Parameters
    ----------
    home_countries : list of str
        ISO2 codes of the countries actually modelled in ``n`` (typically
        ``config["countries"]``), as opposed to the foreign power-pool
        countries added by :func:`add_foreign_buses`.
    """
    buses = n.buses[n.buses["country"].isin(home_countries)].copy()
...
def add_cross_border_links(
    n, power_pool_links, substation_dict, home_countries, distance_crs
):
    """Add cross-border links to the network."""
    for _, row in power_pool_links.iterrows():
        name = row["name"]
        if row["from_country"] in home_countries:
            lat, lon = substation_dict[name]
            bus0 = find_nearest_bus(n, lat, lon, home_countries, distance_crs)
        else:
            bus0 = row["from_country"]
        if row["to_country"] in home_countries:
            lat, lon = substation_dict[name]
            bus1 = find_nearest_bus(n, lat, lon, home_countries, distance_crs)
        else:
            bus1 = row["to_country"]
...
def add_interconnectors(
    ...
    ----------
    home_countries : list of str
        ISO2 codes of the countries modelled in ``n`` (``config["countries"]``).
        Anything else referenced in ``power_pool_links`` is treated as a
        foreign power-pool country and gets its own aggregate bus.
    """
    substation_dict = {
...
def add_custom_line_types(n, custom_line_types):
    """Merge custom line_types into the pypsa network, overriding duplicates."""
